...
3. MissionDefinition (compiled in JAR) deploys each player ship via the V2 placeholder-then-swap pattern: `addToFleet(side, stockVariantId, ...)` returns a `FleetMemberAPI`, then `member.setVariant(VariantBuilder.createVariant(spec), false, true)` swaps in the optimizer-generated variant before deployment. CR is set on the FleetMember via `getRepairTracker().setCR(spec.cr)`. Enemy ships use stock variant IDs via `addToFleet()`. (Earlier V1 path used `VariantBuilder.createFleetMember(spec)` + `addFleetMember(side, member)` to build the variant up-front — reverted 2026-05-10 because deployed ships boot with `retreat=true` set internally and `setRetreating(false, false)` cannot override.)
