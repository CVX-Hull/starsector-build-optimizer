...
| 2026-07-14 | shipped | [Accounting-run stream pre-registration](2026-07-14-accounting-stream-preregistration.md) | Pre-registration + analysis ledger for the roadmap item-3 accounting run: predeclares the Tier-1/Tier-2 gate statistics, the Tier-2 oracle-coverage rank-stratified subset rule, the fresh seeds (hammerhead 100–108, wolf 120–122), and the retained-artifact manifest — committed before the stream is collected (stream-reuse discipline). Entry 3 (2026-07-17) records the first post-collection consultation (the Tier-2 replay reading). |
| 2026-07-17 | shipped | [Phase 7 Tier-2 oracle-value replay](2026-07-17-phase7-oracle-value-replay.md) | Second prequential replay, now with 27-build oracle coverage. The coverage did not certify the surrogate: CatBoost-vs-oracle Spearman +0.34 (n=27, marginal) but ≈0 among rankable/deployable builds; gating median q\*=0.3 = the build-blind null; T2 drift reproduces. The one positive oracle signal is the TWFE α̂ target (+0.5, marginal), not the surrogate. Confirms the shipped "gating value not established" against an independent oracle. All claims exploratory. |
| 2026-07-17 | shipped | [Accounting matchups-per-trial spread](2026-07-17-accounting-matchup-spread.md) | Roadmap item-3 accounting deliverable: a completed trial costs ~10 matchups, a pruned trial ~6 (hammerhead) / ~7 (wolf); dispatched-but-unscored overhead is negligible (0.14%/0.26%) with zero instance-error/worker-timeout trials. Resolves the wolf non-meta hull, unmeasured before this run. Matchup counts only (V2 cost model pending). |
