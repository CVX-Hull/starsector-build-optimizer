...
## Additional findings — 2026-05-09 launcher debug cycle

Surfaced during the smoke #4 → #12 sequence after a Starsector minor-version update broke the Swing-launcher click. Captured here for the same triage discipline as the 2026-04-19 sweep.

### R1 — Launcher click is heuristic; OCR/template fallback would be drift-proof

- **Location:** `src/starsector_optimizer/instance_manager.py::_click_launcher` (Python) and `combat-harness/src/main/java/starsector/combatharness/MenuNavigator.java` (in-game Robot clicks).
- **Finding:** the current launcher dispatch computes the Play-button screen coordinate as `(X + W * launcher_play_button_x_fraction, Y + H * launcher_play_button_y_fraction)` parsed from `xdotool getwindowgeometry --shell <wid>`. Defaults `(0.5, 0.7)` were chosen from smoke #12 evidence and verified to work on Starsector 0.98a-RC8's 597×373 Swing launcher. **The fractions are still a heuristic** — any future game version that reshuffles the launcher layout (extra modal, multi-row button bar, button repositioned) will silently miss the Play button. The `MenuNavigator` in-game clicks are the same shape: hardcoded `int CONTINUE_X = 963, CONTINUE_Y = 892;` calibrated to one specific resolution × game version, retuned manually each version.
- **Observed?** Yes — directly in this debug cycle. The PRIOR static coords `(297, 255)` had been correct on an earlier Starsector version where the launcher opened at the X-display origin (no WM = no auto-center). A 2-3 minor-version update centered the launcher; the old absolute coords landed outside the 597×373 frame, the click never fired, all smoke runs hung at the launcher screen for 2 weeks until the user noticed.
- **Why this matters operationally:** every Starsector update potentially re-breaks both the Python launcher click AND the Java in-game Robot clicks. Re-tuning is currently a manual debug cycle requiring `launcher_dispatch.log` evidence + screenshot calibration. The cycle takes a multi-rebake debug session per release.
- **Proposed fix — staged image-processing fallback:**
  1. **Tier 1 (geometric, current).** Keep as default; cheapest and works on stable layouts. ~5 ms per dispatch.
  2. **Tier 2 (OCR, tesseract).** When Tier 1 fails (post-click `getwindowname` still returns the launcher 2 s later), screenshot via `import` or `xwd`, run `pytesseract.image_to_data` to find the literal text "Play Starsector", click the bounding-box center. Resists graphical redesigns *and* layout shifts because the text content has been stable since 2014. ~0.5–2 s per scan; one-shot per JVM. Requires `tesseract-ocr` apt package + `pytesseract` Python dep in the AMI.
  3. **Tier 3 (template match, OpenCV).** Bake a Play-button reference PNG into the AMI; `cv2.matchTemplate` on the screenshot scores it. Adds `opencv-python-headless`. ~30 ms per match. Brittle to button-graphic redesigns but the Starsector launcher graphic has barely changed in years. Useful as a Tier-2-ish alternative if OCR's Latin-character font rendering proves unreliable on Swing-rendered text.
  4. **`launcher_dispatch.log` already records which tier won** (it logs the dispatched coordinates and the post-click `getwindowname` outcome) — drift over time is observable from real evidence rather than guessed.
- **Why deferred:** Tier 1 currently passes Tier-2 smoke; the rebake-on-version-update cost is bounded and the user has acceptable tolerance for it. Tier 2 (OCR) is the natural next step **the next time a game update breaks the click**, not preemptively — adding tesseract to the AMI now is dead weight until needed. Same logic applies to the in-game Robot coords in `MenuNavigator.java`: defer until the next manual recalibration becomes painful enough to justify the work.
- **Related work:** Phase 7.5 already targets the AMI-bake-per-code-change cycle (Tier A's worker Dockerfile). Adding OCR there is a one-RUN-line change to the worker Dockerfile when Phase 7.5 ships — keep this on the same delivery vehicle.

### R2 — Operator SSH access depends on out-of-band tailnet ACL configuration

- **Location:** `src/starsector_optimizer/cloud_userdata.py::render_user_data` (the `debug_ssh_pubkey` parameter and the `/home/ubuntu/.ssh/authorized_keys` injection block).
- **Finding:** when `STARSECTOR_DEBUG_SSH_PUBKEY` is set on the orchestrator, the worker boots with the matching pubkey in `authorized_keys`, and `ssh -i <key> ubuntu@<worker-tailnet-ip>` works *if* the workstation can reach the tailnet IP. From a userspace-mode `tailscaled` on macOS, this requires `tailscale ssh` (which uses an internal ProxyCommand) but `tailscale ssh` *also* requires the tailnet ACL to grant `ssh` permissions — which a default-permissive personal tailnet does NOT include. Smoke #11–#12 confirmed: `tailscale ping` works (UDP datagrams flow through the wireguard tunnel), but `tailscale ssh ubuntu@<tailnet-ip>` silently times out at port 22 because the ACL forbids it. The same path WOULD work in kernel-mode tailscaled or with an ACL grant for `tag:CVX-Hull@` → `tag:starsector-worker:22`.
- **Observed?** Yes — fully blocked operator SSH access during smoke #11 debug despite all the orchestrator-side plumbing being correct. The eventual unblock was the rebake-and-test cycle on `launcher_dispatch.log` evidence alone.
- **Proposed fix:** Phase 7.5's "Tailscale Terraform provider" item (Tier B, line 187 of `phase7.5-infrastructure-reproducibility.md`) is the right delivery vehicle. The Terraform module should declare a tailnet ACL fragment that grants `ssh` from the workstation tag to the worker tag, alongside the existing tagOwners + grants + authkey declarations. Forked engineers running through Phase 7.5's `terraform apply` would inherit a working ACL, so the SSH path works on the first smoke. Until Phase 7.5 ships, the operator-side workaround is to manually update the tailnet ACL via the Tailscale admin web UI before the first smoke that needs SSH debugging.
- **Why deferred:** the `authorized_keys` injection plumbing is shipped (smoke #11 verified the pubkey lands at `/home/ubuntu/.ssh/authorized_keys`); only the tailnet routing is operator-action. ACL-as-code via the Tailscale Terraform provider is correctly scoped to Phase 7.5; doing it now would mean owning a one-off Terraform configuration outside the Phase 7.5 framework.
- **Related work:** see `phase7.5-infrastructure-reproducibility.md` § Tier B item 6.
