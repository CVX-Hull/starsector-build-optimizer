...
| `eb` | `EBShrinkageConfig` | `EBShrinkageConfig()` | Empirical-Bayes shrinkage parameters for A2′ (enabled switch — **default off** since the 2026-07-13 re-groom, see spec 28 — plus τ² floor, triple-goal toggle, min builds, OLS ridge). |
| `shape` | `ShapeConfig` | `ShapeConfig()` | A3 Box-Cox output-warping parameters. `ShapeConfig(enabled: bool = False, min_samples: int = 8, positivise_epsilon: float = 1e-6)`. `enabled` is **default off** since the 2026-07-13 re-groom (honest-eval end-to-end ranking disfavored the Box-Cox stack; [2026-07-13-roadmap-regroom.md](../reports/2026-07-13-roadmap-regroom.md) D3) — when False, A3 passes the fitness through unchanged with `shape_passthrough_reason: "disabled"`. When enabled: below `min_samples`, A3 falls through to min-max scaling (Box-Cox MLE is unstable under ~8 samples; floor chosen by analogy to `eb_min_builds`); `positivise_epsilon` guards the `shift = min - eps` positivise step and the constant-population `ptp < eps` fallback. |
...
- `_shape_passthrough_reasons: collections.Counter[str]` — histogram of passthrough causes (`"disabled"`, `"n<1"`, `"n<min_samples"`, `"constant"`) for the end-of-run summary
...
**The terminal-failure paths emit NO JSONL row.** There are **two**: `InstanceError` (worker/instance death) and exhausted `WorkerTimeout` retries (a dispatched matchup returned no result within `result_timeout_seconds` after `max_worker_timeout_retries` re-dispatches). Both tell the trial `failure_score` COMPLETE and drop it **without** an `_append_eval_log` row (they produced no valid combat result and no per-opponent data), via the shared `_finalize_terminal_failure(ifb, reason)` helper. This is deliberate: the prequential replay's `(source_path, trial_number)` join is bijective, so a terminal-failure JSONL row with zero matchup-DB rows would orphan the join (spec 31). Their dispatched-matchup count therefore cannot live in the JSONL; instead each records, on the Optuna trial, `set_user_attr("matchups_dispatched", n)` **and** `set_user_attr("terminal_reason", …)` — `"instance_error"` or `"worker_timeout"`. These two finalizers are the **only** callers of `set_user_attr`. Accounting extractors identify terminal-failure trials as `state=COMPLETE` study-DB trials with **no corresponding JSONL row** (the `failure_score` sentinel is NOT a discriminator — invalid-spec shares it but *does* emit a JSONL row) and partition them by `terminal_reason` (absent → `instance_error`, for pre-discriminator runs). `WorkerTimeout` is bounded by `max_worker_timeout_retries` so a never-completing matchup cannot loop forever; a *transient* timeout re-dispatches the same build (stable `matchup_id` drains any late result) and, on eventual success, emits a normal JSONL row.
