...
| `eval_batch_size` | `int` | `8` | Builds evaluated per batch. Set to num_instances for full utilization. |
...
4. `sampler = _create_sampler(config)` — creates TPE or CatCMAwM based on `config.sampler`
