...
- **Phase 7.5** (planned, scheduled after Phase 7 ships): **Infrastructure & Reproducibility**. Horizontal to the algorithmic phases — collapses the 16-step tribal-knowledge bootstrap sequence into a one-command fork-and-go workflow. Four incremental tiers: (A) `just` CLI + worker Dockerfile + `starsector-repro check` preflight + Terraform module for static AWS infra (1–2 weeks, preserves current architecture, eliminates ~80% of AMI rebakes triggered by Python-only changes); (B) Flyte/Prefect flow replacing `launch_campaign.sh` + bash `trap EXIT` + Tailscale Terraform provider + structured `gate.json` (2–4 weeks); (C) Ray Tune adoption replacing `StagedEvaluator` + `CloudWorkerPool` + `worker_agent.py` main loop + reliable-queue plumbing — ~2000 LOC deletion, best pursued AS Phase 7's execution-substrate delivery vehicle (4–8 weeks); (D) Nix flake + published `REPRODUCE.md` with canonical reference run for ecological-reproducibility validation (top-10 Jaccard ≥ 0.5, mean composite diff ≤ 0.02 across independent fresh runs). Motivated by post-Phase-6 retrospective of the 16 tribal-knowledge steps a forked engineer currently has to perform (own AWS account + VPC + subnet IDs hardcoded in Packer, own Tailscale tailnet + manual admin-panel grants, licensed game files + prefs.xml, correct versions of Packer/uv/Python/Java/Redis/Tailscale clients, SSH keypair named `starsector-probe`, implicit knowledge of project_tag/namespace/teardown scope equivalence, ASHA rung vs trial distinction, load_avg ∈ [3, 8] invariant, kernel-vs-userspace tailscale preflight semantics, four-layer teardown semantics). Full design doc + alternative ranking at `docs/reference/phase7.5-infrastructure-reproducibility.md`. Success criterion: forked engineer on unfamiliar laptop reaches passing Tier-2 smoke in under 30 minutes given only the repo URL and a pointer to a legally obtained Starsector copy. Rejected alternatives: Kubernetes/KubeRay (too heavy at <$10k/mo), Airflow/Dagster (DAG-of-different-tasks-oriented), W&B Launch/SageMaker Pipelines (vendor lock-in), Snakemake/Nextflow (bio idioms).
