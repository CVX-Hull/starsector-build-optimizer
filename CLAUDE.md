...
- **Phase 6** (partial infrastructure shipped, 2026-04-18; Tier-1 probe wired): **Cloud Worker Federation**. **Shipped + test-green**: `AWSProvider` (full EC2 Fleet lifecycle including per-campaign LaunchTemplate + SecurityGroup creation + base64-encoded UserData + tag-scoped teardown that reaps instances+LT+SG together), `cloud_userdata.render_user_data` (umask 0077 before writing `/etc/starsector-worker.env`; Tailscale authkey piped via stdin, never argv, to avoid `/proc/<pid>/cmdline` leak), `CostLedger` (append-only JSONL with `fsync` per row, `BudgetExceeded` at hard cap, warn thresholds `(0.5, 0.8, 0.95)`), Packer AMI template (`scripts/cloud/packer/aws.pkr.hcl`; baked AMIs 2026-04-18: us-east-1=`ami-0106d8575802f9941`, us-east-2=`ami-028c38dfe92e71939`), and `scripts/cloud/probe.{sh,py}` (Tier-1: exercises `AWSProvider` end-to-end, provisions → RUNNING → terminate → `final_audit.sh`; ~$0.05 per run). **Not yet end-to-end**: `CampaignManager.provision_fleet` raises `NotImplementedError` — a campaign-wide fleet cannot carry study-specific bearer tokens and Redis queue keys without leaking every study's config to every worker, so per-study fleet provisioning (each study subprocess renders its own WorkerConfig → UserData and drives `AWSProvider.create_fleet`) lands in the smoke-test scope. The probe bypasses `CampaignManager` entirely and drives `AWSProvider` directly. Topology (unchanged from design): **workstation = orchestrator** (holds every Optuna Study; subprocess-per-`(hull, regime, seed)`); workers = pure `MatchupConfig → CombatResult` evaluators running on AWS c7a.2xlarge spot via `worker_agent.py`. Orchestrator ↔ worker protocol: Redis `BRPOPLPUSH` source→processing reliable-queue (janitor thread re-queues stuck items after `visibility_timeout_seconds`) + per-study Flask listener (`POST /result` only; bearer token + dedup by `matchup_id`); all workstation-worker traffic over Tailscale. `EvaluatorPool` ABC (`evaluator_pool.py`) is the cross-backend contract — `LocalInstancePool` and `CloudWorkerPool` are the two implementations behind it; `StagedEvaluator` depends only on the ABC. `CloudProvider` ABC ships with `AWSProvider` and `HetznerProvider` NotImplementedError stub (unlock at $500+). Three-layer teardown design = in-process `try/finally` + `atexit.register` + shell `trap EXIT` (probe uses all three; campaign flow will inherit once provision_fleet is wired). Packer AMI (`scripts/cloud/packer/aws.pkr.hcl`) bakes game files, combat-harness mod, uv venv, Tailscale client, `x11-xserver-utils` for the LWJGL XRandR warmup (2.2-2.4× local per-instance once the Xvfb `xrandr --query` warmup runs in `_start_xvfb`); `bake_image.sh` copies us-east-1 AMI → us-east-2 via `aws ec2 copy-image`. No backward compat — pre-Phase-6 `scripts/cloud/deploy.sh` / `run.sh` / `collect.sh` / `sync-code.sh` / `cloud-init.yaml` / `aws/*.sh` are deleted; Phase 6 scripts are flat under `scripts/cloud/`. Secrets hygiene: `CampaignConfig.__repr__` redacts `tailscale_authkey_secret`, `WorkerConfig.__repr__` redacts `bearer_token`; neither is serialized into the cost ledger or study DB. Workers never import `repair` (AST-enforced test) — repair stays orchestrator-side per Phase 1-4 optimizer-space / domain-space boundary. Deferred from Phase 6 MVP: plateau detector (hardcoded trial cap for first campaign), tag-based sweeper cron, CloudWatch billing alarm (orthogonal operational backstops). **Budget (when run): $85 total** = $1.35 probe + smoke + $14.83 sampler benchmark + $60.79 prep + $5 slack (figures pinned by `experiments/phase6-planning/cost_model.py`). Prep campaign scope: 8 hulls × `early` regime × 1 seed × ~600 trials ≈ 48,000 matchups ≈ 4.1 hr wall-clock at 96 VMs; hulls span FRIGATE→CAPITAL to give Phase 7's attribute-kernel + per-hull role-mode mixture weights the cross-size transfer evidence. SOP: `.claude/skills/cloud-worker-ops.md`. Full design + rejected-alternative chain: `docs/reference/phase6-cloud-worker-federation.md`. Spec: `docs/specs/22-cloud-deployment.md`.
