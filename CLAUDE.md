...
- **Phase 6** (infrastructure complete, 2026-04-18): **Cloud Worker Federation**. Python modules + spec 22 rewrite + Packer AMI template + operator scripts shipped; validation probe / pipeline smoke / sampler benchmark / Phase 7 prep campaign deferred to operational sessions. Topology: **workstation = orchestrator** (holds every Optuna Study; subprocess-per-`(hull, regime, seed)`); workers = pure `MatchupConfig → CombatResult` evaluators running on AWS c7a.2xlarge spot (us-east-1 + us-east-2) via `worker_agent.py`. Orchestrator ↔ worker protocol: Redis `BRPOPLPUSH` source→processing reliable-queue (janitor thread re-queues stuck items after `visibility_timeout_seconds`) + per-study Flask listener (`POST /result` only; bearer token + dedup by `matchup_id`); all workstation-worker traffic over Tailscale. `EvaluatorPool` ABC (`evaluator_pool.py`) is the cross-backend contract — `LocalInstancePool` (renamed from `InstancePool`) and `CloudWorkerPool` are the two implementations behind it; `StagedEvaluator` depends only on the ABC. `CloudProvider` ABC (`cloud_provider.py`) ships with `AWSProvider` (boto3 direct EC2 Fleet with `price-capacity-optimized` + `CapacityRebalancing`) and `HetznerProvider` NotImplementedError stub (unlock at $500+). `CampaignManager` (`campaign.py`) is a supervisor: provisions EC2 Fleet, spawns subprocess-per-study, enforces `budget_usd` via `CostLedger` (append-only JSONL with `fsync` per row; `BudgetExceeded` at hard cap; warn thresholds `(0.5, 0.8, 0.95)`); three-layer teardown = `try/finally` + `atexit.register` + shell `trap EXIT` in `launch_campaign.sh`. Packer AMI (`scripts/cloud/packer/aws.pkr.hcl`) bakes game files, combat-harness mod, uv venv, Tailscale client, `x11-xserver-utils` for the LWJGL XRandR warmup (2.2-2.4× local per-instance once the Xvfb `xrandr --query` warmup runs in `_start_xvfb`); `bake_image.sh` copies us-east-1 AMI → us-east-2 via `aws ec2 copy-image`. No backward compat — pre-Phase-6 `scripts/cloud/deploy.sh` / `run.sh` / `collect.sh` / `sync-code.sh` / `cloud-init.yaml` / `aws/*.sh` are deleted; Phase 6 scripts are flat under `scripts/cloud/`. Secrets hygiene: `CampaignConfig.__repr__` redacts `tailscale_authkey_secret`, `WorkerConfig.__repr__` redacts `bearer_token`; neither is serialized into the cost ledger or study DB. Workers never import `repair` (AST-enforced test) — repair stays orchestrator-side per Phase 1-4 optimizer-space / domain-space boundary. Deferred from Phase 6 MVP: plateau detector (hardcoded trial cap for first campaign), tag-based sweeper cron, CloudWatch billing alarm (orthogonal operational backstops). **Budget (when run): $85 total** = $1.35 probe + smoke + $14.83 sampler benchmark + $60.79 prep + $5 slack (figures pinned by `experiments/phase6-planning/cost_model.py`). Prep campaign scope: 8 hulls × `early` regime × 1 seed × ~600 trials ≈ 48,000 matchups ≈ 4.1 hr wall-clock at 96 VMs; hulls span FRIGATE→CAPITAL to give Phase 7's attribute-kernel + per-hull role-mode mixture weights the cross-size transfer evidence. SOP: `.claude/skills/cloud-worker-ops.md`. Full design + rejected-alternative chain: `docs/reference/phase6-cloud-worker-federation.md`. Spec: `docs/specs/22-cloud-deployment.md`.
