...
class TestEndToEndPipeline:
    ...
    def eagle_builds(self, game_data):
        eagle = game_data.hulls["eagle"]
        return generate_diverse_builds(eagle, game_data, 200, seed=123)

    def test_all_repaired_builds_feasible(self, game_data, eagle_builds):
        eagle = game_data.hulls["eagle"]
    ...
    def test_all_builds_within_op_budget(self, game_data, eagle_builds):
        eagle = game_data.hulls["eagle"]
        for build in eagle_builds:
            cost = compute_op_cost(build, eagle, game_data)
            assert cost <= eagle.ordnance_points

    def test_all_builds_slot_compatible(self, game_data, eagle_builds):
        eagle = game_data.hulls["eagle"]
    ...
    def test_all_builds_scoreable(self, game_data, eagle_builds):
        eagle = game_data.hulls["eagle"]
        for build in eagle_builds:
    ...
    def test_all_builds_generate_valid_variants(self, game_data, eagle_builds):
        eagle = game_data.hulls["eagle"]
        for build in eagle_builds:
    ...
    def test_all_builds_extract_features(self, game_data, eagle_builds):
        eagle = game_data.hulls["eagle"]
        for build in eagle_builds:
...
class TestMultiHullPipeline:
    ...
    def test_build_score_variant_for_hull(self, game_data, hull_id):
        if hull_id not in game_data.hulls:
            pytest.skip(f"{hull_id} not in game data")
        hull = game_data.hulls[hull_id]
        builds = generate_diverse_builds(hull, game_data, 20, seed=42)
        for build in builds:
            ok, violations = is_feasible(build, hull, game_data)
