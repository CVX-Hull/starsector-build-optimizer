...
Scope (per docs/reference/phase6-cloud-worker-federation.md §10):
  - IAM surface (create_fleet, terminate, describe)
  - AMI availability in every targeted region
  - LaunchTemplate + SecurityGroup creation
  - Tag propagation
  - Terminate-all-tagged actually terminates
  - list_active → [] after teardown
...
_RUN_DEADLINE_SECONDS = 600.0   # 10 min; create_fleet → all running
...
def _poll_until_running(
    provider: AWSProvider, campaign_name: str, target: int, deadline: float,
) -> list[dict]:
    while time.monotonic() < deadline:
        active = provider.list_active(campaign_name)
...
def _poll_until_empty(
    provider: AWSProvider, campaign_name: str, deadline: float,
) -> None:
    while time.monotonic() < deadline:
        active = provider.list_active(campaign_name)
        if not active:
...
def run_probe(config_path: Path) -> int:
    ...
    try:
        logger.info("=== phase 1: create_fleet ===")
        instance_ids = provider.create_fleet(config, user_data=user_data)
        if not instance_ids:
            raise RuntimeError("create_fleet returned zero instance IDs")
        ...
        active = _poll_until_running(
            provider, config.name, target=len(instance_ids), deadline=deadline,
        )
        ...
        logger.info("=== phase 4: teardown ===")
        try:
            terminated = provider.terminate_all_tagged(config.name)
            logger.info("terminated: %d instances", terminated)
        except Exception as e:
            logger.error("terminate_all_tagged raised: %s", e)

        logger.info("=== phase 5: verify list_active == [] ===")
        try:
            deadline = time.monotonic() + _TEARDOWN_DEADLINE_SECONDS
            _poll_until_empty(provider, config.name, deadline)
